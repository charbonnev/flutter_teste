...
class SIAColetaSupabaseUser extends BaseAuthUser {
  SIAColetaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> sIAColetaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = SIAColetaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
